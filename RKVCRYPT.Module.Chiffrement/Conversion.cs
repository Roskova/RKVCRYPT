...
namespace RKVCRYPT.Module.Chiffrement
{
    ...
    {
        //Génère la table de chiffrement
        ...
        private string Space(string message, int count)
        {
            int l = 0;
            for (int i = count - 1; i < message.Length; i += count)
            {
                message = message.Insert(i, " ");
                if (i * 100 / message.Length != l)
                {
                    l = i * 100 / message.Length;
        ...
        {
            string patternTrue = @"^[AB]*$";
            string patternFalse = @"^[0-6]*$";
            Regex t = new Regex(patternTrue);
            Regex t2 = new Regex(patternFalse);
            if (type && t.IsMatch(message))
            {
        ...
        internal string ProtocoleBinarosk(string message, bool type)
        {
            ...
            if (type) //Conversion en binarosk
            {
                ...
                {
                    message = message.Replace("1", "BAAA").Replace("2", "ABAA").Replace("3", "AABA").Replace("4", "AAAB").Replace("5", "BBAA").Replace("6", "AABB").Replace("7", "BABB").Replace("8", "BBAB").Replace("9", "BAAB").Replace("0", "ABBA");
                }
            }
